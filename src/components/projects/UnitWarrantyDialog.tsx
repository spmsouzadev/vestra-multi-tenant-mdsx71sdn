...
export function UnitWarrantyDialog({
  ...
  const [masterDate, setMasterDate] = useState(new Date().toISOString().split('T')[0])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Garantias Configuradas', description: `Garantias aplicadas para ${units.length} unidade(s).` })
      onSuccess()
      onOpenChange(false)
    } catch (error) {
      toast({ variant: 'destructive', title: 'Erro', description: 'Falha ao gerar garantias.' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Aplicando para {units.length} unidade(s). Selecione as categorias e defina as datas de início.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4 py-4 flex-1 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-1">
                ...
                <Button variant="ghost" size="sm" onClick={toggleAll} className="h-auto p-0 text-primary">
                    {categories.every(c => selection[c.id]) ? 'Desmarcar Todas' : 'Selecionar Todas'}
                </Button>
            </div>
                ...
                <div className="space-y-2">
                    {categories.length === 0 && (
                        <div className="text-center py-8 text-muted-foreground">
                            Nenhuma categoria configurada. Vá em Configurações > Garantias para criar.
                        </div>
                    )}
                    {categories.map(cat => {
                        const isSelected = !!selection[cat.id]
                        return (
                            <div key={cat.id} className={`flex items-center gap-4 p-3 rounded-md border transition-colors ${isSelected ? 'bg-white border-primary/50' : 'bg-slate-50 border-transparent'}`}>
                                <Checkbox 
                                    id={cat.id} 
                                    checked={isSelected}
                                ...
                                <div className="flex-1">
                                    <Label htmlFor={cat.id} className="font-medium cursor-pointer">{cat.name}</Label>
                                    <p className="text-xs text-muted-foreground">{cat.termMonths} meses</p>
                                </div>
                                {isSelected && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Input 
                                            ...
                                            onChange={(e) => updateCategoryDate(cat.id, e.target.value)}
        ...
        <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={loading || Object.keys(selection).length === 0}>
